...
(define-data-var staking-contract-address principal .staking-v1) 
...
(define-public (add-rewards-sbtc
  ...
  (let (
    ...
    (try! (contract-call? .sbtc-token transfer sbtc-amount contract-caller (as-contract tx-sender) none))

    (if (> pool-owner-amount u0)
      ;; TODO: update with mainnet sbtc token
      (as-contract (contract-call? .sbtc-token transfer pool-owner-amount tx-sender (get receiver pool-owner-commission) none))
      (ok true)
    )
  )
)
...
(define-public (get-sbtc (requested-sbtc uint) (receiver principal))
  (begin
    ...
    ;; TODO: update with mainnet sbtc token
    (try! (as-contract (contract-call? .sbtc-token transfer requested-sbtc tx-sender receiver none)))
